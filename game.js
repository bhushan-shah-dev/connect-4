...
const gameState = {
    ...
    set currentPlayer(currentPlayer) {
        ...
        body.style.boxShadow = 'inset 0 0 100px var(--player-' + currentPlayer + '-color)';
    },
    board: new Array(NUM_COLUMNS).fill(null).map(() => new Array(NUM_ROWS).fill(null))
};

const boardEventTarget = new EventTarget();
boardEventTarget.addEventListener('fillSlot', function (event) {
    // Update gameState
    gameState.board[event.detail.column][event.detail.slotToFill] = event.detail.player;

    // Update DOM
    document.getElementById('board').children[event.detail.column].children[event.detail.slotToFill].style.backgroundColor = 'var(--player-' + event.detail.player + '-color)'
});
...
function setupBoard() {
    ...
    for (let i = 0; i < NUM_COLUMNS; i++) {
        ...
        for (let j = 0; j < NUM_ROWS; j++) {
            ...
            slot.className = 'slot';
            slot.addEventListener('click', () => dropDisc(i, j), { signal: slotController.signal });
...
function dropDisc(column, row) {
    ...
    boardEventTarget.dispatchEvent(new CustomEvent('fillSlot', {
        detail: {
            column,
            slotClicked: row,
            slotToFill,
...
function checkWinner(column, row) {
    ...
    const currentDiagonal1 = gameState.board.map((c, i) => c[column + row - i]).filter(val => typeof val !== 'undefined');
    const currentDiagonal2 = gameState.board.map((c, i) => c[i + row - column]).filter(val => typeof val !== 'undefined');
